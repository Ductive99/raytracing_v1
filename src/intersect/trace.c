...
/*   Updated: 2025/12/19 20:57:57 by esouhail         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (hit->obj_type == OBJ_PLANE || hit->obj_type == OBJ_CYLINDER_CAP)
		get_plane_uv(hit->point, hit->normal, u, v);
...
t_color	trace_ray(t_ray ray, t_scene *scene)
{
	...
	if (closest.hit)
	{
		...
		return (calculate_lighting(scene, closest, ray.direction));
	}
	bg.r = 0;
	bg.g = 0;
	bg.b = 0;
